...
char* indexes(const char* path)
{
    //if a path exists, check if index.php or index.html exist
...
{ 
    ...
    //create buffer to store bytes from file
    BYTE buffer[BYTES];
    //read all bytes of file into buffer
    fread(buffer, sizeof(buffer), BYTES, file);
    //store address of first byte
    *content = &buffer[0];
